...
**Objectives**: 
  Exploring and generating statistics of the data using R libraries.
...
print(paste0("The number of rows in the region data are", b[1]))
print(paste0("The number of columns in the region data are", b[2]))
```
...
```{r}
summary(athletes_df$Medal)
`


```{r}
dim(athletes_df$Medal)
```

```{r}
print(paste0("The total gold medal count is ", summary(athletes_df$Medal)[1]))
```
